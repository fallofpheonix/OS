...
import (
	"github.com/fallofpheonix/phoenix/foundation/runtime/bus/normalizer"
	"testing"
	"time"
)
...
func TestCalculateEntropy(t *testing.T) {
	...
	uniform := []byte("aaaa")
	entropy := normalizer.CalculateEntropy(uniform)
	if entropy != 0.0 {
	...
	entropy = normalizer.CalculateEntropy(varied)
	if entropy < 4.0 {
	...
	maxEntropy := normalizer.CalculateEntropy(allBytes)
