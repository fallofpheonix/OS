 ...
 */
func (mse *MultiSensorEvidence) CalculateConfidence(sensorWeights map[string]float64) ConfidenceScore {
	if len(mse.Observations) == 0 {
		return 0
	}

	// Group observations by value to find consensus
	valueGroups := make(map[string]float64)
	...
	for sensorID, obs := range mse.Observations {
		...
		valueGroups[string(obs.Value)] += weight
		totalWeight += weight
	}
	...
	mse.ConfidenceScore = ConfidenceScore(maxWeight / totalWeight)
...
func (mse *MultiSensorEvidence) IsValid(minSensors int, minConfidence ConfidenceScore) bool {
	...
	if mse.ConfidenceScore < minConfidence {
		return false
	}
	return true
}
