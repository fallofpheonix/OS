...
import (
	...
	"github.com/fallofpheonix/phoenix-os/phoenix_os/ai/agents"
	"github.com/fallofpheonix/phoenix-os/phoenix_os/kernel/sandbox"
	ledger "github.com/fallofpheonix/phoenix-os/phoenix_os/truth_ledger/src"
	"github.com/fallofpheonix/phoenix-os/phoenix_os/security/control"
	"github.com/fallofpheonix/phoenix-os/phoenix_os/security/game/stackelberg"
	"github.com/fallofpheonix/phoenix-os/phoenix_os/security/physics"
	"github.com/fallofpheonix/phoenix-os/phoenix_os/telemetry/detector"
	"github.com/fallofpheonix/phoenix-os/phoenix_os/telemetry/events"
	lineage "github.com/fallofpheonix/phoenix-os/phoenix_os/telemetry/process_lineage"
)
...
func main() {
	...
	for {
		...
		if newState != prevState {
			...
			if action != "NONE" {
				evidence := ledger.LedgerEntry{
					TraceHash:  evt.EventID,
					EventID:    evt.EventID,
					Source:     "telemetry_replay",
					Payload:    []byte(fmt.Sprintf("SDI:%.2f,Action:%s,Confidence:%.2f", physState.Disorder, action, res.ImportanceScore)),
