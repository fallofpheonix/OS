...
func (a *CryptoAuditor) AuditHash(data []byte, context string) string {
	...
	// Record to ledger
	_ = a.ledger.AddEntry("CRYPTO-OP", "SHA256", 0, []byte(fmt.Sprintf("ctx:%s,hash:%s", context, hashStr)))
...
func (a *CryptoAuditor) AuditVerify(expected, actual, context string) bool {
	...
	_ = a.ledger.AddEntry("CRYPTO-VERIFY", context, 0, []byte(fmt.Sprintf("match:%v", match)))
	return match
}
