...
func (b *Bus) Publish(topic string, event TelemetryEvent) {
	...
	for _, ch := range subs {
		...
		// 0.8 is 800000 with precision 1000000
		thresh80 := phxmath.NewFixedPoint(8)
		thresh80.V = thresh80.V / 10 // 0.8

		// 0.9 is 900000 with precision 1000000
		thresh90 := phxmath.NewFixedPoint(9)
		thresh90.V = thresh90.V / 10 // 0.9

		if fillRatio >= HighWatermark {
			if event.Severity.V < thresh80.V {
				b.Dropped++
				continue
			}
		}
		...
		if fillRatio >= CriticalWatermark {
			...
			if event.Severity.V < thresh90.V {
				b.Dropped++
				continue
			}
		} else {
