 ...
 */
// =========================================================================
// WORKFLOW POSITION: CYCLE 9d — ISOLATION ENGINE SNAPSHOT (Layer 4.5)
//
// The IsolationEngine snapshot system captures the containment state
// for persistence and recovery. It provides:
//   - CreateSnapshot(): serialize current state with integrity hash
//   - RestoreFromSnapshot(): deserialize and validate state
//
// WORKFLOW:
//   CreateSnapshot() → serialize CurrentState + History → SHA-256 hash
//     → JSON marshal → return []byte
//   RestoreFromSnapshot(data) → unmarshal → validate hash → restore state
//
// INTEGRITY: The snapshot hash is computed over the state, history, version,
// and sequence. Any tampering will be detected during restoration.
//
// VERSIONING: Snapshots include a version string (CurrentVersion = "1.0.0").
// Incompatible versions are rejected during restoration.
// =========================================================================
package containment
